"""
summarizer.py
Generates a plain-English summary of legal document text
...
def summarize_document(text: str, model: str = "llama3") -> dict:
    """
    Sends document text to Ollama (Llama 3) and returns a plain-English summary.
    ...
    Returns:
        A dict with key "summary" (str).
        On failure, returns {"summary": "", "error": "..."}.
    """
    ...
    try:
        ...
        if "summary" not in result or not isinstance(result["summary"], str):
            return {"summary": "", "error": "Model response missing 'summary' key."}

        return result

    except json.JSONDecodeError:
        return {"summary": "", "error": f"Failed to parse JSON: {raw[:200]}"}
    except requests.ConnectionError:
        return {"summary": "", "error": "Cannot connect to Ollama. Is it running?"}
    except Exception as e:
        return {"summary": "", "error": f"API call failed: {str(e)}"}
